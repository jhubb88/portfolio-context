...
Originally a single workflow in `jhubb88/ClaudeCode/.github/workflows/deploy.yml` deployed advanced-projects + 5 standalone projects (text-to-audio, log-analyzer, resume-matcher, traffic-dashboard, ntcip-simulator) by checking out *stale stub copies* under `Desktop/ClaudeCode/<project>/`. Bug found 2026-04-30 (advanced-projects); audit 2026-05-01 confirmed the same bug class on the other 5. **All 6 migrations complete 2026-05-01.** Each project now has its own `deploy.yml` in its standalone repo; ClaudeCode's deploy.yml retains only the `jimmy-cloudfront-distributions` CloudFormation step (the entire "Invalidate CloudFront caches" step was removed since no S3 deploys remain).
...
- **AI Resume Matcher (fixed 2026-05-01):** `jhubb88/resume-matcher/.github/workflows/deploy.yml` — push to main → `aws s3 sync . s3://jimmy-resume-matcher/ --delete` (same excludes as log-analyzer: `.git/*`, `.github/*`, `*.md`, `LICENSE`, `.gitignore`, `deploy.sh`, `lambda_function.py`) → invalidate `E3H0XAJDR3BQG1`. README frontend-sync block replaced with CI/CD note; `./deploy.sh` infrastructure block intact. Note: backend Lambda also writes to `jimmy-resume-matcher-results` bucket (results storage, separate from frontend bucket).
- **Live Traffic Dashboard (fixed 2026-05-01):** `jhubb88/traffic-dashboard/.github/workflows/deploy.yml` — push to main → `aws s3 sync . s3://jimmy-traffic-dashboard/ --delete` (excludes `.git/*`, `.github/*`, `*.md`, `LICENSE`, `.gitignore`, **`.superpowers/*`, `bucket-policy.json`, `diagrams/*`, `docs/*`** — repo includes brainstorm artifacts, S3 policy, architecture SVGs, and planning docs that should not land on the frontend bucket) → invalidate `E3H1V6C42HG9P1`. README replaced manual deploy block with CI/CD note. Note: `diagrams/*.svg` files are excluded here because they're served from `jimmy-writing-workspace/diagrams/` for the WordPress portfolio page (single canonical source).
- **NTCIP Simulator (fixed 2026-05-01 — last one):** `jhubb88/ntcip-simulator/.github/workflows/deploy.yml` — push to main → `aws s3 sync . s3://jimmy-ntcip-simulator/ --delete` (excludes `.git/*`, `.github/*`, `*.md`, `LICENSE`, `.gitignore`, **`generate_icons.py`** — local icon-generation script) → invalidate `E2PEIMT1J3W4MO`. README replaced manual deploy block with CI/CD note. PWA assets (manifest.json, sw.js, icon-192.png, icon-512.png) all at repo root and deployed. The `--delete` flag also wiped the stale `icons/` subdirectory from S3 (canonical removed it in commit `e0d38bd`; SW cache key `ntcip-sim-v2` unchanged). Note: S3 default Content-Type now applies (`manifest.json` → `application/json`, `sw.js` → `text/javascript`); previously ClaudeCode workflow set `application/manifest+json` and `application/javascript` explicitly via `aws s3 cp --content-type`. Both work for PWA.
- **FieldIQ (fixed 2026-05-01):** `jhubb88/FieldIQ/.github/workflows/prewarm.yml` — now triggers on push to main (fast deploy-only path, ~1 min) AND weekly Sun 02:00 UTC schedule + `workflow_dispatch` (full prewarm + deploy, ~17 min). Setup Node + prewarm + auto-commit-cache steps gated with `if: github.event_name == 'schedule' || github.event_name == 'workflow_dispatch'` so push events skip the CFBD API marathon. Sync now uses `--delete` with tightened excludes (`*.md`, `LICENSE`, `.gitignore` added; `.playwright-mcp/*`, `.codex`, `*.pdf`, `*.csv`, `p17-*.png`, `ohio-state-*.png`, `screenshot-*.png` removed since they're not committed and `--delete` cleans them off S3). Initial deploy removed 24 files (~10.5 MB) from S3 plus 5 stragglers via manual `aws s3 rm` (FieldIQ_Master.md, NOTES.md, README.md, LICENSE, .gitignore). `config.js` (CFBD API key) intentionally preserved by exclude — required at runtime by `index.html` → `js/api.js`. Queued follow-up: have workflow generate `config.js` from `secrets.CFBD_API_KEY` at deploy time + optionally rotate the current key.
- **RAG Knowledge Chatbot (added 2026-05-04 — completes CI/CD migration, 9 of 9):** `jhubb88/aws-rag-chatbot/.github/workflows/deploy.yml` — push to main with changes under `frontend/**` or the workflow file → `aws s3 sync frontend/ s3://rag-chatbot-603509861186-dev/ --exclude ".gitkeep"` → invalidate `EN88LEBW14923` with `/*` paths. **No `--delete` flag** — bucket is shared with RAG infrastructure (`documents/` 61MB vector index, `lambda/` deployment packages, `cloudtrail/` audit logs); a delete-flagged sync would break the query Lambda. Workflow file carries an explicit warning above the sync step. README got a new `## Deployment` section. Frontend is single-file static (Scenario A, no build step), hardcoded IDs (CloudFormation stack `rag-knowledge-chatbot` has drifted — Phase 12 realignment deferred — so workflow does not read from stack outputs). Note: GitHub auto-trigger did not fire on the introductory commit `e0d91a9` (known GitHub Actions quirk for workflow-introduction commits with paths filters); manual `workflow_dispatch` ran end-to-end successfully. Subsequent pushes touching `frontend/**` or the workflow itself trigger normally.
- **Linux Ops Command Copilot (fixed 2026-05-01):** `jhubb88/linux-ops-command-copilot/.github/workflows/deploy.yml` — push to main → `npm ci` → `npm run build` (Vite + React 19 + TS, produces `dist/`) → reads `BucketName` and `DistributionId` from the `linux-ops-copilot-frontend` SAM stack outputs (`aws cloudformation describe-stacks`) → `aws s3 sync dist/ s3://$BUCKET --delete` → invalidate `$DIST_ID`. Different shape from other portfolio fixes: needs build step (Scenario B), and bucket/distribution IDs are read from CloudFormation stack outputs instead of hardcoded — matches the existing `scripts/deploy-frontend.sh` pattern (preserved as manual fallback). Bucket: `linux-ops-copilot-frontend-603509861186` (private, OAC-fronted), CloudFront `E39D8FLKEFZ16I` → `d1dqp0w50lre0j.cloudfront.net`. SAM template at `frontend/template.yaml` manages infra separately. README got a new `## Deployment` section. `.env.production` (committed, contains `VITE_API_URL`) gets baked into the bundle at build time.
